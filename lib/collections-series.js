...
export function buildCollectionsPayload({ window, currentRows, previousRows }) {
  ...
  const currentSeries = buildMonthSeries({
    ...
    throughDay: window.currentThroughDay,
  ...
  const monthToDate = seriesTotalThrough(
    currentSeries,
    window.currentThroughDay
  );
  ...
  return {
    ...
    reporting_cutoff_date: window.reportingCutoffDate,
    settlement_lag_business_days: window.settlementLagBusinessDays,
    current_month: {
      label: window.currentLabel,
      month_to_date: monthToDate,
      reported_through: window.currentEnd,
