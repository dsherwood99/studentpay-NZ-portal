...
test('Pacific/Auckland date is used instead of UTC calendar day', () => {
  ...
  assert.equal(window.reportingCutoffDate, '2026-08-18');
  assert.equal(window.currentEnd, '2026-08-18');
  assert.equal(window.currentThroughDay, 18);
  assert.equal(window.samePeriodDay, 18);
  assert.equal(window.settlementLagBusinessDays, 5);
...
test('31 March cutoff uses 24 March, not today, for the same-period day', () => {
  const window = getReportingWindow(new Date('2026-03-30T12:00:00.000Z'));

  assert.equal(window.generatedOn, '2026-03-31');
  assert.equal(window.reportingCutoffDate, '2026-03-24');
  assert.equal(window.currentEnd, '2026-03-24');
  assert.equal(window.currentThroughDay, 24);
  assert.equal(window.previousDaysInMonth, 28);
  assert.equal(window.samePeriodDay, 24);
  assert.equal(window.previousEnd, '2026-02-28');
});

test('early-month cutoff can leave the current series empty', () => {
  const window = getReportingWindow(new Date('2026-09-02T12:00:00.000Z'));

  assert.equal(window.generatedOn, '2026-09-03');
  assert.equal(window.reportingCutoffDate, '2026-08-27');
  assert.equal(window.currentEnd, null);
  assert.equal(window.currentThroughDay, 0);
  assert.equal(window.samePeriodDay, 27);
});
