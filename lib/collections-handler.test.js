...
test('provider A cannot retrieve provider B collections', async () => {
  ...
  assert.equal(result.body.reporting_cutoff_date, '2026-08-18');
  assert.equal(result.body.settlement_lag_business_days, 5);
  assert.equal(result.body.current_month.reported_through, '2026-08-18');
});
...
test('collections query uses the cutoff as the current-month end date', async () => {
  const calls = [];
  const result = await read(
    await handleCollectionsRequest({
      user: providerUser('Online Learning Institute'),
  ...
  assert.equal(result.status, 200);
  assert.equal(calls[0].endDate, '2026-08-18');
  assert.equal(calls[0].startDate, '2026-08-01');
  assert.equal(calls[1].startDate, '2026-07-01');
  assert.equal(calls[1].endDate, '2026-07-31');
  assert.equal(result.body.reporting_cutoff_date, '2026-08-18');
});
